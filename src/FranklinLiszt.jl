module FranklinLiszt
    ...
    * `{{...}}` to produce a sublist.
    """
    function format_list_entry(data, key)
        list_entry = ""
        if startswith("{{", key)
            key = strip(key, ['{','}'])
            startswith(key, "ol ") && list_entry = """$(list_entry)
                $(hfun_ol(strip(key[3:end])))
                """
            startswith(key, "ul ") && list_entry = """$(list_entry)
                $(hfun_ul(strip(key[3:end])))
                """
            startswith(key, "dl ") && list_entry = """$(list_entry)
                $(hfun_dl(strip(key[3:end])))
                """
        else
            key_parts = split(keys,":")
            list_entry = """$(list_entry)
                    $(format_entry(
                        data,
                        first(key_parts);
                        class= length(key_parts) > 1 ? key_parts[2] : first(key_parts),
                        url = length(key_parts) > 2 ? key_parts[3] : ""
                    ))"""
        end
        return list_entry
    end
    ...
    """
    function format_entry(data, kay; class=key, url="")
